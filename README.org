...
*** To Do
**** Project
- [ ] Split scheduler dependencies out into submodules of ~common-lander~
- [ ] Concurrency pass
  - Need to register all systems with minimal dependencies to prevent overconstraint
- [ ] Delete old ~cl_schedule~ and replace with ~_cl_schedule~
...
- [ ] Investigate unary function syntax abuse
  - ~printf "foo"~ is valid Lua
  - ~set{1, 2, 3}~ is also valid Lua, where ~set~ is a unary function
- [ ] Formalize use of ~__index~ and ~__newindex~ and ~__metatable~ for wrapper types
...
- [ ] Factor out ~goldenorb_joint~
  - Only exists as a dependency for the non-working offset aim code now ~goldenorb_recoil~ exists as an example of spring usage
